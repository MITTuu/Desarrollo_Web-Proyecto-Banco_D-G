// authentication.js - Integrado con el Backend
import { api, setAuthToken, setCurrentUser, removeAuthToken } from './api-config.js';
...
function showLoading(loadingEl, callback) {
  ...
  // Retornar función para ocultar loading
  return () => {
...
loginForm.addEventListener('submit', async (e) => { 
  ...
  try {
    ...
    if (response.success && response.data) {
      ...
      loginError.textContent = response.message || 'Error al iniciar sesión';
    }
...
const registerFields = [
  ...
  { el: registerUsername, validator: () => validateUsername(registerUsername.value), message: "Username inválido (4-20 caracteres)" },
...
registerForm.addEventListener('submit', async (e) => {
  ...
  try {
    ...
    const nombreCompleto = fullname.value.trim().split(' ');
    const nombre = nombreCompleto[0];
    const apellido = nombreCompleto.slice(1).join(' ') || nombre;

    // Mapear tipo de identificación a UUID (estos son ejemplos, ajusta según tu BD)
    const tipoIdMap = {
      'nacional': 'e6292fb0-f15b-4998-a5d4-ccdad6949215', // Ajustar con el UUID real
      'dimex': '695fe4b3-3752-4c4b-99ad-501a443e8d0c',
      'pasaporte': 'b690305c-dab2-4de7-a29e-7fddae4f56cd'
    };

    // Llamar al endpoint de registro
    const response = await api.post('/users', {
      tipoIdentificacion: tipoIdMap[idType.value],
      identificacion: idNumber.value,
      nombre: nombre,
      apellido: apellido,
      correo: email.value,
      telefono: phone.value || null,
      usuario: registerUsername.value,
      password: registerPassword.value,
      rol: '00f26bba-b055-4d84-a4e3-7919e12a9a61' // UUID del rol "cliente" - ajustar según tu BD
    }, {
      requiresAuth: false,
      useApiKey: true
    });
...
const recoverFields = [
  { el: document.getElementById("recover-identifier"), validator: v => !!v.trim(), message: "Ingrese su correo electrónico" },
  { el: document.getElementById("recover-code"), validator: v => /^\d{6}$/.test(v.trim()), message: "Código debe tener 6 dígitos" },
...
document.getElementById("sendCodeBtn").addEventListener("click", async () => {
  ...
  if(!input.value.trim()){ 
    ...
    error.textContent="Ingrese su correo electrónico"; 
    return; 
  }
  ...
  hideLoading = showLoading(loadingRecover);
  ...
  try {
    ...
    if (response.success) {
      ...
      alert(response.message || 'Error al enviar código');
    }
  } catch (error) {
    hideLoading();
...
document.getElementById("verifyCodeBtn").addEventListener("click", async () => {
  ...
  const emailInput = document.getElementById("recover-identifier");
  const error = input.nextElementSibling;
  
  if(!/^\d{6}$/.test(input.value.trim())){ 
    input.classList.add("error"); 
    error.textContent="Código debe tener 6 dígitos"; 
    return; 
  }
  ...
  try {
    ...
    console.error("Error:", error);
    input.classList.add("error");
    error.textContent = error.message || "Código inválido";
  }
});
